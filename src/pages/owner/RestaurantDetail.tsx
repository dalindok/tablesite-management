...
function HoursTab({ restaurantId }: { restaurantId: number }) {
  ...
  const defaultHours = (): HourRow[] =>
    DAYS.map((d) => ({ day_of_week: d, open_time: "09:00", close_time: "22:00", is_closed: false }));
  ...
  useEffect(() => {
    setLoading(true);
    restaurantsApi.ownerGetFull(restaurantId).then((r) => {
      const oh = r.data.data.operating_hours;
      if (oh && oh.length > 0) {
        setHours(DAYS.map((d) => {
          const found = oh.find((h) => h.day_of_week === d);
          return found
            ? { day_of_week: d, open_time: found.open_time, close_time: found.close_time, is_closed: found.is_closed }
  ...
  const update = (day: DayOfWeek, field: keyof HourRow, value: string | boolean) =>
  ...
  if (loading)
    return <PageLoading compact title="Loading hours…" />;
    ...
    <SectionCard
      ...
      action={
        <button className="btn-primary text-sm" onClick={save} disabled={saving}>
          {saving ? <Spinner /> : saved ? <RiCheckLine size={16} /> : <RiSaveLine size={16} />}
    ...
    >
      <div className="space-y-2">
        {hours.map((h) => (
          ...
          >
            <span className="font-medium text-slate-700 text-sm">{DAY_LABELS[h.day_of_week]}</span>
...
function TablesTab({ restaurantId }: { restaurantId: number }) {
  ...
  // Form state lives directly here — NOT in a sub-component (avoids focus-loss on re-render)
  const [form, setForm] = useState<CreateTablePayload>({ table_number: "", capacity: 2 });
  ...
  const startEdit = (t: Table) => {
    setEditRow(t);
    setForm({ table_number: t.table_number, capacity: t.capacity, floor: t.floor, description: t.description });
    setAddOpen(false);
  };
  ...
  const save = async () => {
    ...
    try {
      if (editRow) {
        await restaurantsApi.ownerUpdateTable(restaurantId, editRow.id, form as UpdateTablePayload);
        setEditRow(null);
      } else {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await restaurantsApi.ownerDeleteTable(restaurantId, deleteTarget);
  ...
  // Inline form — no sub-component to avoid focus loss
  const renderForm = () => (
    <div className="grid grid-cols-2 gap-3 p-4 rounded-xl border border-primary-200 bg-primary-50">
      <div>
        <label className="label text-xs">Table # *</label>
        <input className="input-field py-1.5 text-sm" placeholder="T1"
          value={form.table_number}
          onChange={(e) => setForm((f) => ({ ...f, table_number: e.target.value }))} />
      </div>
      <div>
        <label className="label text-xs">Seats *</label>
        <input type="number" min={1} className="input-field py-1.5 text-sm"
          value={form.capacity}
          onChange={(e) => setForm((f) => ({ ...f, capacity: +e.target.value }))} />
      </div>
      <div>
        <label className="label text-xs">Floor</label>
        <input className="input-field py-1.5 text-sm" placeholder="Ground Floor"
          value={form.floor ?? ""}
          onChange={(e) => setForm((f) => ({ ...f, floor: e.target.value }))} />
      </div>
      <div>
        <label className="label text-xs">Description</label>
        <input className="input-field py-1.5 text-sm" placeholder="Near window, corner table…"
          value={form.description ?? ""}
          onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} />
      </div>
      <div className="col-span-2 flex gap-2 justify-end">
        <button className="btn-secondary text-sm" onClick={() => { setAddOpen(false); setEditRow(null); }}>
          <RiCloseLine size={15} /> Cancel
        </button>
        <button className="btn-primary text-sm" onClick={save} disabled={saving}>
          {saving ? <Spinner /> : <RiCheckLine size={15} />}
          {editRow ? "Save Changes" : "Add Table"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <>
      <SectionCard
        title="Tables"
        action={
          <button className="btn-primary text-sm" onClick={startAdd} disabled={addOpen || !!editRow}>
            <RiAddLine size={15} /> Add Table
          </button>
        }
      >
        {addOpen && <div className="mb-4">{renderForm()}</div>}

        {loading ? (
          <PageLoading compact title="Loading tables…" />
        ) : tables.length === 0 && !addOpen ? (
          <p className="text-center text-slate-400 py-8 text-sm">No tables yet. Add one above.</p>
        ) : (
          <div className="space-y-2">
            {tables.map((t) =>
              editRow?.id === t.id ? (
                <div key={t.id} className="mb-2">{renderForm()}</div>
              ) : (
                <div key={t.id}
                  className="grid grid-cols-[80px_70px_100px_1fr_auto] gap-3 items-center p-3 rounded-xl border border-slate-200 bg-white hover:border-slate-300 transition-colors">
                  <span className="font-semibold text-slate-800 text-sm">{t.table_number}</span>
                  <span className="text-slate-600 text-sm">{t.capacity} seats</span>
                  <span className="text-slate-500 text-xs">{t.floor ?? "—"}</span>
                  <span className="text-slate-400 text-xs truncate">{t.description ?? "—"}</span>
                  <div className="flex gap-1">
                    <button className="p-1.5 rounded-lg text-slate-400 hover:bg-blue-50 hover:text-blue-500 transition-colors" onClick={() => startEdit(t)}>
                      <RiEditLine size={15} />
                    </button>
                    <button className="p-1.5 rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors" onClick={() => setDeleteTarget(t.id)}>
      ...
      <ConfirmDialog
        ...
        loading={deleting}
        title="Delete Table"
        message={`Delete table "${tables.find(t => t.id === deleteTarget)?.table_number}"? This cannot be undone.`}
        confirmLabel="Delete"
      />
    </>
  );
...
function GalleryTab({ restaurantId }: { restaurantId: number }) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<number | null>(null);
  const [deleting, setDeleting]   = useState(false);
  const [form, setForm]           = useState<AddGalleryImagePayload>({ url: "", caption: "", sort_order: 0 });
  ...
  const add = async () => {
    ...
    setSaving(true);
    try {
      await restaurantsApi.ownerAddGalleryImage(restaurantId, { ...form, sort_order: images.length });
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await restaurantsApi.ownerDeleteGalleryImage(restaurantId, deleteTarget);
      setDeleteTarget(null);
      load();
    } catch (e: any) {
  ...
  return (
    <>
      <SectionCard title="Gallery Images">
        {/* Add form — Caption above URL */}
        <div className="space-y-3 mb-5 p-4 rounded-xl border border-slate-200 bg-slate-50">
          <div>
            <label className="label text-sm">Caption</label>
            <input className="input-field text-sm" placeholder="e.g. Main dining hall, Rooftop view…"
              value={form.caption ?? ""}
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input className="input-field flex-1 text-sm" placeholder="https://…"
                value={form.url}
                onChange={(e) => setForm((f) => ({ ...f, url: e.target.value }))} />
              <button className="btn-primary text-sm" onClick={add} disabled={saving || !form.url}>
        ...
        {loading ? (
          <PageLoading compact title="Loading gallery…" />
        ) : images.length === 0 ? (
          <p className="text-center text-slate-400 py-8 text-sm">No images yet. Paste a URL above.</p>
        ) : (
...
function TagsClosuresTab({ restaurantId }: { restaurantId: number }) {
  ...
  const [saving, setSaving]   = useState(false);
  const [cForm, setCForm]     = useState<CreateClosurePayload>({ date: "", reason: "" });
  const [cSaving, setCsaving] = useState(false);
  const [closureDeleteTarget, setClosureDeleteTarget] = useState<number | null>(null);
  const [closureDeleting, setClosureDeleting]         = useState(false);
  ...
  const confirmDeleteClosure = async () => {
    if (closureDeleteTarget == null) return;
    setClosureDeleting(true);
    try {
      await restaurantsApi.ownerDeleteClosure(restaurantId, closureDeleteTarget);
      setClosureDeleteTarget(null);
      load();
    } catch (e: any) {
      alert(e?.response?.data?.message ?? "Error deleting closure");
    } finally {
      setClosureDeleting(false);
    }
  };

  if (loading)
    return <PageLoading compact title="Loading tags & closures…" />;
    ...
    <>
      <div className="space-y-5">
        ...
        <SectionCard
          title="Tags"
          action={
            <button className="btn-primary text-sm" onClick={saveTags} disabled={saving}>
        ...
        >
          <div className="flex gap-2 mb-4">
            <input className="input-field flex-1 text-sm" placeholder="e.g. Fine Dining"
              value={tagInput}
          ...
          {tags.length === 0 ? (
            <p className="text-center text-slate-400 text-sm py-4">No tags. Type above and press Enter.</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {tags.map((tag) => (
                <span key={tag} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary-50 text-primary-700 text-sm font-medium">
                  {tag}
                  <button onClick={() => removeTag(tag)} className="text-primary-400 hover:text-red-500 transition-colors">
                    <RiCloseLine size={14} />
                  </button>
                </span>
              ))}
        ...
        <SectionCard title="Special Closures">
          <div className="flex flex-col gap-2 mb-4">
            <input className="input-field text-sm" placeholder="Reason (e.g. Khmer New Year)"
              value={cForm.reason ?? ""}
              onChange={(e) => setCForm((f) => ({ ...f, reason: e.target.value }))} />
            <div className="flex gap-2">
              <input type="date" className="input-field text-sm flex-1"
                value={cForm.date}
                onChange={(e) => setCForm((f) => ({ ...f, date: e.target.value }))} />
              <button className="btn-primary text-sm" onClick={addClosure} disabled={cSaving || !cForm.date}>
                {cSaving ? <Spinner /> : <RiAddLine size={15} />} Add
              </button>
            </div>
          </div>
          {closures.length === 0 ? (
            <p className="text-center text-slate-400 text-sm py-4">No special closures.</p>
          ) : (
            <div className="space-y-2">
              {closures.map((c) => (
                <div key={c.id} className="flex items-center justify-between p-3 rounded-xl border border-slate-200 bg-white">
                  ...
                  <button
                    ...
                    onClick={() => setClosureDeleteTarget(c.id)}>
                    <RiDeleteBinLine size={15} />
                  </button>
                </div>
              ))}
      ...
      <ConfirmDialog
        isOpen={closureDeleteTarget != null}
        onClose={() => setClosureDeleteTarget(null)}
        onConfirm={confirmDeleteClosure}
        loading={closureDeleting}
        title="Remove Closure"
        message={`Remove the closure for "${closures.find(c => c.id === closureDeleteTarget)?.date?.toString().split("T")[0] ?? ""}"?`}
        confirmLabel="Remove"
      />
    </>
  );
...
function MenusTab({ restaurantId }: { restaurantId: number }) {
  ...
  const [menuForm, setMenuForm] = useState<CreateMenuPayload>({ name: "", is_active: true, sort_order: 0 });
  const [menuAdding, setMenuAdding] = useState(false);
  const [itemForm, setItemForm] = useState<CreateMenuItemPayload>({
    name: "", price: 0, category: "", is_available: true,
    is_vegan: false, is_vegetarian: false, is_gluten_free: false, sort_order: 0,
  });
  const [itemAdding, setItemAdding] = useState(false);
  const [saving, setSaving]     = useState(false);

  // Delete confirm state
  const [menuDeleteTarget, setMenuDeleteTarget] = useState<number | null>(null);
  const [menuDeleting, setMenuDeleting]         = useState(false);
  const [itemDeleteTarget, setItemDeleteTarget] = useState<{ menuId: number; itemId: number } | null>(null);
  ...
  const addMenu = async () => {
    ...
    setSaving(true);
    try {
      await menusApi.create(restaurantId, { ...menuForm, sort_order: menus.length });
  ...
  const confirmDeleteMenu = async () => {
    if (menuDeleteTarget == null) return;
    setMenuDeleting(true);
    try {
      await menusApi.delete(restaurantId, menuDeleteTarget);
      if (activeMenu?.id === menuDeleteTarget) setActiveMenu(null);
  ...
  const toggleActive = async (menu: Menu) => {
    try {
      await menusApi.update(restaurantId, menu.id, { is_active: !menu.is_active });
      await load();
    } catch (e: any) {
  ...
  const addItem = async () => {
    ...
    setSaving(true);
    try {
      await menusApi.createItem(restaurantId, activeMenu.id, { ...itemForm, sort_order: activeMenu.items?.length ?? 0 });
      setItemForm({ name: "", price: 0, category: "", is_available: true, is_vegan: false, is_vegetarian: false, is_gluten_free: false, sort_order: 0 });
      setItemAdding(false);
      await load();
    } catch (e: any) {
  ...
  const confirmDeleteItem = async () => {
    ...
    try {
      await menusApi.deleteItem(restaurantId, itemDeleteTarget.menuId, itemDeleteTarget.itemId);
  ...
  return (
    <>
      <div className="grid grid-cols-[260px_1fr] gap-5 items-start">
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="text-primary-500 hover:text-primary-700 transition-colors" onClick={() => setMenuAdding((v) => !v)}>
              <RiAddLine size={18} />
            </button>
          </div>
          ...
          {menuAdding && (
            <div className="mb-3 space-y-2 p-3 rounded-xl border border-primary-200 bg-primary-50">
              <input className="input-field text-sm py-1.5" placeholder="Menu name"
                value={menuForm.name}
                onChange={(e) => setMenuForm((f) => ({ ...f, name: e.target.value }))} />
              <input className="input-field text-sm py-1.5" placeholder="Description (optional)"
                value={menuForm.description ?? ""}
                onChange={(e) => setMenuForm((f) => ({ ...f, description: e.target.value }))} />
              <div className="flex gap-1.5">
                <button className="btn-primary py-1.5 text-xs flex-1" onClick={addMenu} disabled={saving}>
                  {saving ? <Spinner /> : "Add"}
                </button>
                <button className="btn-secondary py-1.5 text-xs" onClick={() => setMenuAdding(false)}>Cancel</button>
              </div>
            </div>
          )}

          {loading ? (
            <PageLoading compact title="Loading menus…" />
          ) : menus.length === 0 ? (
            <p className="text-center text-slate-400 text-xs py-4">No menus yet.</p>
          ) : (
              ...
              {menus.map((m) => (
                <div key={m.id}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400">{m.items?.length ?? 0} items</p>
                  </div>
                  <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      ...
                      onClick={(e) => { e.stopPropagation(); setMenuDeleteTarget(m.id); }}>
        ...
        <div className="card p-5">
          {!activeMenu ? (
            ...
            <>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-semibold text-slate-700">{activeMenu.name}</h3>
                  {activeMenu.description && <p className="text-xs text-slate-400 mt-0.5">{activeMenu.description}</p>}
                </div>
                <button className="btn-primary text-sm" onClick={() => setItemAdding((v) => !v)}>
                ...
                <div className="mb-4 p-4 rounded-xl border border-primary-200 bg-primary-50 space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      ...
                      <input className="input-field text-sm py-1.5" placeholder="Fish Amok"
                        value={itemForm.name}
                        onChange={(e) => setItemForm((f) => ({ ...f, name: e.target.value }))} />
                    </div>
                    <div className="col-span-2">
                      <label className="label text-xs">Description</label>
                      <input className="input-field text-sm py-1.5" placeholder="Short description"
                        value={itemForm.description ?? ""}
                        onChange={(e) => setItemForm((f) => ({ ...f, description: e.target.value }))} />
                    </div>
                    <div>
                      <label className="label text-xs">Price ($) *</label>
                      <input type="number" step="0.01" min="0" className="input-field text-sm py-1.5"
                        value={itemForm.price}
                        onChange={(e) => setItemForm((f) => ({ ...f, price: +e.target.value }))} />
                    </div>
                    <div>
                      <label className="label text-xs">Category</label>
                      <input className="input-field text-sm py-1.5" placeholder="Main / Appetizer / Dessert"
                        value={itemForm.category ?? ""}
                        onChange={(e) => setItemForm((f) => ({ ...f, category: e.target.value }))} />
                    </div>
                    <div className="col-span-2">
                      <label className="label text-xs">Image URL</label>
                      <input className="input-field text-sm py-1.5" placeholder="https://…"
                  ...
                  <div className="flex gap-2">
                    <button className="btn-primary text-sm" onClick={addItem} disabled={saving}>
                      {saving ? <Spinner /> : "Add Item"}
                    </button>
                    <button className="btn-secondary text-sm" onClick={() => setItemAdding(false)}>Cancel</button>
                  </div>
                </div>
              )}
              ...
              {(activeMenu.items?.length ?? 0) === 0 && !itemAdding ? (
                <p className="text-center text-slate-400 py-10 text-sm">No items. Add one above.</p>
              ) : (
                <div className="space-y-2">
                    ...
                    <div key={item.id}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {!item.is_available && (
                            <span className="text-xs px-2 py-0.5 rounded-full bg-red-50 text-red-500">Unavailable</span>
                          )}
                        </div>
                        {item.description && (
                        ...
                        <div className="flex gap-2 mt-1">
                          {item.is_vegan && <span className="text-xs text-green-600">Vegan</span>}
                          {item.is_vegetarian && <span className="text-xs text-emerald-600">Vegetarian</span>}
                          {item.is_gluten_free && <span className="text-xs text-blue-600">GF</span>}
                        </div>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="font-semibold text-slate-800">${Number(item.price).toFixed(2)}</p>
                        <button
                          className="mt-1 p-1 rounded text-slate-400 hover:text-red-500 hover:bg-red-50 transition-colors"
                          onClick={() => setItemDeleteTarget({ menuId: activeMenu.id, itemId: item.id })}>
                          <RiDeleteBinLine size={14} />
                        </button>
                      </div>
                    </div>
      ...
      {/* Delete menu confirm */}
      <ConfirmDialog
        isOpen={menuDeleteTarget != null}
        onClose={() => setMenuDeleteTarget(null)}
        onConfirm={confirmDeleteMenu}
        loading={menuDeleting}
        title="Delete Menu"
        message={`Delete "${menus.find(m => m.id === menuDeleteTarget)?.name}" and all its items? This cannot be undone.`}
        confirmLabel="Delete"
      />
...
export default function RestaurantDetail() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    restaurantsApi.ownerGetFull(restaurantId)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => navigate("/owner/restaurants")}
        ...
        <div>
          {loading ? (
            ...
            <>
              ...
              <p className="text-sm text-slate-400">{info?.cuisineType} · {info?.city}</p>
            </>
          )}
        </div>
      </div>
      ...
      {/* Tab content */}
      {loading && <PageLoading compact title="Loading restaurant…" />}
